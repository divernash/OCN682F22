...
```{r}
# new_data=mpg
# sorted=factor(new_data$class, ordered=TRUE)
# new_data$class_numeric=sapply(sorted,as.integer,use.names=FALSE)
# is.integer(new_data$class_numeric)
```
...
```{r}
# new_data=mpg
# ggplot(new_data,aes(cty,hwy,color=class))+geom_point(size=6)+geom_smooth(method="lm", color="red")
```
...
```{r}
# mpgsubset=subset(mpg, mpg$cty==15, manufacturer:model)
# mpgsubset
```
...
# ggplot(data=mpg)+
#  geom_point(mapping=aes(x=displ, y=hwy, shape=class))+
#   scale_shape_manual(values=rep(0:7))
```
...
# ggplot(sizeMass, aes(Size, Mass))+geom_point()+geom_smooth(method="lm", color="red", se=FALSE)
```
...
# x2=log10(x)
# y2=log10(y)
# q8_new=data.frame(x2,y2)
# colnames(q8_new)=c("size_log10", "mass_log10")
# ggplot(q8_new, aes(size_log10, mass_log10))+geom_point()+geom_smooth(method="lm", color="red", se=FALSE)
