...
# coral=sample(coral_pop, size=100,replace=TRUE, prob=NULL)
# is_diseased=sample(x=c("TRUE",FALSE), size=100, replace=TRUE, prob=c(0.2, 0.8))
...
```{r}
# sample(rnorm(100,10,1),size=5,replace=TRUE,prob=NULL)
```
...
```{r}
# binary_health=function(x){if(x==TRUE){as.integer(TRUE)}else{as.integer(FALSE)}}
# binary_health=mapply(binary_health, coral_health_tbl$is_diseased)
# binary_heath_status=cbind(coral_health_tbl,binary_health)
```
...
```{r}
# ggplot(coral,aes(x=coral_cover,y=stat(count/sum(count)),fill=is_diseased))
#  + geom_histogram(color="black",binwidth=1)
#   + scale_fill_manual(values=c("#FFFFFF,"#7E7E7E"))
#    + labs(title="Total Coral Coverage (Healthy vs. Diseased)",x=Coral Cover, y="Normalized Proportion",fill="Health #      Status)
```
...
```{r}
# ggplot(coral,aes(x=coral_cover,fill=is_diseased))
# + geom_density(adjust=2,alpha=0.5)
#  + scale+x_continuous(limits=c(-15,20))
#   + scale_fill_manual(values=c("#FFFFFF,"#7E7E7E"))
#    + labs(title="Total Coral Coverage (Healthy vs. Diseased)",x=Coral Cover, y="Normalized Proportion",fill="Health #      Status)
```
...
```{r}
# https://docs.google.com/spreadsheets/d/1qQJt701PRJ-AzG2AFy7bmkm5fXjPRvuDHVb_QhP3oV4/edit#gid=0

# Tidy data would require more columns. Breaking the (Genus_name/Species_name) column into two separate columns makes # it clearer. Also, removing the first letter of the Genus from the species column cleans it up because the Genus is # already mentioned in another column. Adding a location column cleaned it up . Given that this data is from a single # group of observations the data can be made into one table. 
```
